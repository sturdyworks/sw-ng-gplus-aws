...
.factory('UserService', function($q, $http, AWSService, StripeService) {
  var service = {
    _user: null,
    UsersTable: "Users",
    UsersItemsTable: "UsersItems",
    UsersChargesTable: "UsersCharges",
    Bucket: 'sturdyworks-org',
    ...
    itemsForSale: function() {
      ...
      service.currentUser().then(function(user) {
        AWSService.dynamo({
          params: {TableName: service.UsersItemsTable}
        }).then(function(table) {
          table.query({
            TableName: service.UsersItemsTable,
            KeyConditions: {
              "user_email": {
    ...
    createPayment: function(item, charge) {
      ...
      .then(function(data) {
        ...
        AWSService.sqs(
          {QueueName: service.UsersChargesTable}
        ).then(function(queue) {
          queue.sendMessage({
